...
Proxy pool selection rotates per probe. max_attempts defaults to 1 (range 1–20); rejected 11/13-block states and ordinary network errors advance through the pool, sharing timeout_seconds (default 15, range 1–180). HTTP 429 / rate_limit_exceeded / invalid_api_key abort the remaining exits and back off the whole account for rate_limit_backoff_seconds (default 600). usage_limit_reached and insufficient_quota still use quota_backoff_seconds (default 900). Attempts are serialized per account; attempt_pause_ms (default 2000) spaces exits. retry_seconds defaults to 60; refresh_before_seconds defaults to 300. A literal {session} in a secret URL is replaced with eight random hexadecimal characters on each connection.
